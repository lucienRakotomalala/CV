...
\cvitem{Systèmes linéaires continus}{Analyse de systèmes linéaires dynamiques représentés par fonction différentielle ou foncion de transfert.\newline Réalisation en modèle d'état pour une commande par retour d'état. \newline 
Applications de méthodes de discrétisation de systèmes ou de commande pour une implémentation sur carte programmable.}
\cvitem{Systèmes à évènements discrets}{Modélisation, étude et commande de SED en réseau de PETRI, automates et Machine à états.}
...
\cvitemwithcomment{MATLAB}{Etude de systèmes dynamiques, interface graphique GUI, prototypages avec SIMULINK}{Avancé}
\cvitem{Langage C}{Résolution de problèmes mathématiques avec des méthodes numériques. Mise en oeuvre de commande de SED.}
\cvdoubleitem{Editeur de texte}{Microsoft Office et \LaTeX}{OS}{MAC OS, Windows  et Linux}
\cvdoubleitem{Git}{Suivi de projet avec GitHub via l'interface graphique GitKraken}{Raspberry PI}{Mise en place d'un serveur local avec Appache}
\cvlistitem{Analyse et réalisation d'ordonnanceur temps réel pour $\mu $C et carte programmable}

\section{Langues}
\cvitemwithcomment{Anglais}{lu et écrit}{Voyages dans pays anglophones}
\cvitemwithcomment{Occitan}{}{Bilingue pendant l'enfance}
...
\subsection{Musique }
\cvlistitem{Musique : trompette dans un groupe de +20 musciens}
\cvlistitem{Professeur particulier de musique}
\subsection{Sport}
\end{document}
